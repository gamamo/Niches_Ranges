...
# Load dataset --------------------------------------
dataGlobal <- read_csv(here("products","RESULT_areasCVs_global_moll_06_02_2025.csv"));dim(dataGlobal)
dataGlobal[dataGlobal$higher_plant_group=="Ferns and allies","higher_plant_group"] <- "Ferns and lycophytes"


# how many species?
dataGlobal |> dplyr::group_by(higher_plant_group) |> 
  count(higher_plant_group)

# get mean and max niche breadth and range sizes by latitude. These are just descriptive numbers to use in the text
# niche breadth

dataGlobal |>  filter(convex !=0 ) |> 
  group_by(cLat) |> 
  summarise(maxConvex = max(convex),mConvex=mean(convex)) |> filter(maxConvex !=0) -> cv_values_lat

# range sizes

dataGlobal |>  filter(convex !=0 ) |> 
  group_by(cLat) |> 
  mutate(area10 = log10(area)) |> 
  summarise(maxRange = max(area10),mRange=mean(area10)) |> 
  filter(maxRange !=0) -> range_values_lat

# niche breadth mean per species

dataGlobal |>  filter(convex !=0 ) |> 
  group_by(higher_plant_group) |> 
  summarise(medConvex = median(convex),mConvex=mean(convex))

dataGlobal |>  select(convex) |>filter(convex >0.1 ) |>  summary()

#range size per plant group
dataGlobal |>  filter(area >100 ) |> 
  group_by(higher_plant_group) |> 
  summarise(medConvex = median(area),mConvex=mean(area))


dataGlobal |>  select(area) |>filter(area >100 ) |>  summary()


### 1) MAPS: make niche breadth and range size global maps ---------------------------
#### NICHE --------------------------------------------------------------------------
# define color palette
col <- colorRampPalette(c('grey90','steelblue4','steelblue1','gold','red1','red4'))

# load world map and crop to the main land masses
w <- vect(ne_countries())
e <- c(-150, 150, -60, 83.64513)
w <- crop(w,e)

# global map
d0 <-  rast(here("products","MAP_global_CVmean_mosaic_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507) # crop to the world map extension
d0 <-crop(d0,e)

g0 <- ggplot() +
  geom_spatraster(data=d0)+
  theme_bw()+
  scale_fill_gradientn(colors = col(1000),
                       na.value = "transparent",
                       limits=c(0,62),
                       transform="sqrt",
                       breaks=c(0,5,25,50),
                       name="Niche \nbreadth")+
  #               ,limits=c(0,1),breaks=c(0,0.5,1),  transform="sqrt")+
...
d1 <- rast(here("products","MAP_global_CVmean_mosaic_BRYOPHYTES_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507 ) # crop to the world map extension
d1 <- crop(d1,e)

bryopic <- get_uuid(name="Andreaeaceae", n = 1)
g1 <- ggplot() +
  geom_spatraster(data=d1)+
  theme_minimal()+
  scale_fill_gradientn(colors = col(1000),
                       na.value = "transparent",limits=c(0,62),transform="sqrt",
                       breaks=c(0,5,25,50),
                       name="Niche breadth")+
                       #,limits=c(0,1),breaks=c(0,0.5,1),  transform="sqrt")+
  geom_spatvector(data = w, fill = NA)+
  ...
  add_phylopic(uuid = bryopic, x = 13000000, y = 8000000, ysize = 5500000)
g1

# maps Pteridophytes
d2 <- rast(here("products","MAP_global_CVmean_mosaic_FERNS_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507 ) # crop to the world map extension
d2 <- crop(d2,e)

fernpic <- get_uuid(name="Blechum", n = 1)
g2 <- ggplot() +
  geom_spatraster(data=d2)+
  theme_minimal()+
  scale_fill_gradientn(colors = col(1000),
                       na.value = "transparent",limits=c(0,62),transform="sqrt",
                       breaks=c(0,5,25,50),
                       name="Niche breadth")+
                       #transform="sqrt",limits=c(0,1),breaks=c(0,0.5,1))+
  geom_spatvector(data = w, fill = NA)+
  theme(#legend.title = element_blank(),
    axis.title = element_blank(),
    axis.text = element_blank(),
    ...
    legend.title.position = "top",
    legend.position = "bottom")+
  labs(title="Ferns and lycophytes")+
  add_phylopic(uuid = fernpic, x = 13000000, y = 8600000, ysize = 5000000)
g2

# maps gymmnosperms
d3 <- rast(here("products","MAP_global_CVmean_mosaic_GYMNOSPERMS_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507 )
d3 <- crop(d3,e) # crop to the world map extension

gypic <- get_uuid(name="Sequoia sempervirens", n = 1)
g3 <- ggplot() +
  geom_spatraster(data=d3)+
  theme_minimal()+
  scale_fill_gradientn(colors = col(10000),
                       na.value = "transparent",limits=c(0,62),transform="sqrt",
                       breaks=c(0,5,25,50),
                       name="Niche breadth")+
                      # transform="sqrt",limits=c(0,1),breaks=c(0,0.5,1))+
  ...
  add_phylopic(uuid = gypic, x = 13000000, y = 8000000, ysize = 6000000)
g3

# map flowering plants
d4 <- rast(here("products","MAP_global_CVmean_mosaic_ANGIOSPERMS_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507 ) # crop to the world map extension
d4 <- crop(d4,e)
...
g4 <- ggplot() +
  ...
  theme(#legend.title = element_blank(),
    ...
    legend.title.position = "top",
    legend.position = "bottom")+
  labs(title="Flowering plants")+
  add_phylopic(uuid = treepic, x = 13000000, y = 8600000, ysize = 5000000)
g4
...
cp <- g0 + g1 + g2 + g4 + g3 +
  ...
  guide_area() 
ggsave(here("products","RESU_general_MAP_Groups_v2.jpeg"),plot=cp,
       units="cm",dpi = 300,height = 18,width = 30)

cp=(g1 + g2)/ (g4 + g3) +
  plot_annotation(tag_levels = "a",tag_suffix = ")")
ggsave(here("products","RESU_general_MAP_Groups_v3.jpeg"),
...
# global map
d5 <- rast(here("products","MAP_global_Rangemean_mosaic_moll.tif"))
e <- c(-12020987, 15094363, -6074433, 8397507) # crop to the world map extension
d5 <-crop(d5,e)
...
g5 <- ggplot() +
  ...
  theme_bw()+
  scale_fill_gradientn(colors = col(10000),na.value = "transparent",
                       transform="sqrt",
                       labels=label_number(scale = 1/10000),
                       breaks=c(5000,10000000,25000000),
                       name="Range \nsize")+
  geom_spatvector(data = w, fill = NA)+
  theme(axis.title = element_blank(),
        axis.text = element_blank(),
...
##### save map range size and niche breadth all species ------------------------------------------------------

g5 + g0+
  plot_annotation(tag_levels = "a",tag_suffix = ")")
  #plot_layout(guides = "collect")+
  #guide_area() 
ggsave(here("products","RESU_general_MAP_allspecies.jpeg"),
       units="cm",dpi = 300,height = 15,width = 30)

# Analysis -----------------------------------------------------------

d <- dataGlobal |>
  unite(geo,region,band,sep = "_",remove = F ) |> 
  dplyr::filter(convex!=0) |> 
  dplyr::filter(!is.na(ele )) # revisar porque algumas plantas nao tem elevacao...

d[which(d$elevation<0),"elevation"] <- 0
d["Scale_elevation"] <- scale(d$elevation,scale=T, center=F)
d[which(d$region=="Temperate"),"region"] <- "Extratropical"
d[which(d$region=="Temperate/Tropical"),"region"] <- "Latitudinal generalists"
d$region <- as.factor(d$region)
d$region <- fct_relevel(d$region,"Latitudinal generalists")
d[,"area10"] <- log10(d$area)
d[,"convex2"] <- sqrt(d$convex)

d <- d |> mutate(ele2 = case_when(
  elevation < 1000 ~ "Low elevation",
  #elevation > 500 & elevation < 2000 ~"Mid-elevation",
  elevation > 1000 ~ "Mountain"))

source("function_classify_Ele.R")
d$cEle <- classify_values_continuous(d$elevation)

d <- d |> 
  mutate(l = ifelse(ele2 == "Low elevation", "Low elevation", NA)) |> 
  mutate(m = ifelse(ele2 == "Mountain", "Mountain", NA)) |> 
  mutate(e = "Elevation")

d <- d |> 
  mutate(br = ifelse(higher_plant_group == "Bryophytes", "Bryophytes", NA)) |> 
  mutate(fl = ifelse(higher_plant_group == "Ferns and lycophytes", "Ferns", NA)) |> 
  mutate(fp = ifelse(higher_plant_group == "Flowering plants", "Flowering plants", NA),
         gy = ifelse(higher_plant_group == "Gymnosperms", "Gymnosperms", NA)) |> 
  mutate(hpg = "Plant groups")

d <- d |> 
  mutate(tr = ifelse(region == "Tropical", "Tropical", NA)) |> 
  mutate(et = ifelse(region == "Extratropical", "Extratropical", NA)) |> 
  mutate(bo = ifelse(region == "Latitudinal generalists", "Latitudinal generalists", NA)) |> 
  mutate(r = "region")

# get occ records

occ1 <- read_csv(here("products","Occ_per_species.csv"),show_col_types = F)
occ2 <- read_csv(here("products","Occ_per_species_53262.csv"),show_col_types = F)
occ3 <- read_csv(here("products","Occ_per_species_74589.csv"),show_col_types = F)
occ4 <- read_csv(here("products","Occ_per_species_94457.csv"),show_col_types = F)
occ <- rbind(occ1,occ2,occ3,occ4)

d <- left_join(d,occ, by="species")
...
ggsave(here("products","RESU_general_latitude_RANGES.jpeg"),
...
m1bg <-db |> 
  ...
  annotate("text",label = expression(conditional~R^2 == 0.54),
  ...
  annotate("text",label = expression(conditional~R^2 == 0.51),
           x=1,y=8,size=5,hjust = 0)
m1bg
...
m1fg <-df |> 
  ...
  scale_y_continuous(limits=c(1,8))+
  scale_x_continuous(limits=c(1,8))+
  annotate("text",label = expression(conditional~R^2 == 0.57),
           x=0,y=8,size=5,hjust = 0)+
  #legend
  annotate("segment", y = 2.5, yend = 2.5, x = 5.5, xend = 6.1, 
           color = "#56b4e9",linewidth = 1)+
  ...
  annotate("text",label = expression(conditional~R^2 == 0.56),
           x=1,y=8,size=5,hjust = 0)
m1fg
...
m1gg <-dg |> 
  ...
  annotate("text", label="slope = 0.69", x = 7.2, y =1.5,size=5)+
  annotate("text",label = expression(conditional~R^2 == 0.48),
           x=1,y=8,size=5,hjust = 0)
m1gg
...
## 2) model 2: niche breadth per region and band + area --------------------------------
...
drs <- d |> 
  ...
  annotate("text",label = expression(beta ~"1" ~ "= 2516" ~";" ~ beta ~"2 = -2668"),
...
hy <- read_csv("Hypers_CooperEtAl.csv")

hy$Species <- gsub(" ","_",hy$Species)
hy[hy$Continent=="Amazonia","Continent"] <- "Americas"
hy[hy$Continent=="Southeast Asia","Continent"] <- "Asia"

data_hy <-  d|> mutate(source ="Non-dominants")
data_hy <- as.data.frame(data_hy)
write_csv(data_hy,"speciesListToCheck.csv")

#species not in the hyperlist
notListed <- hy[!hy$Species %in% data_hy$species,"Species"]
write_csv(notListed, "hyNotListed.csv")

data_hy[data_hy$species %in% hy$Species,"source"] <- "Hyperdominants"
data_hy[data_hy$band =="Americas","band"] <- "Tropical America"
data_hy[data_hy$band =="Africa/Europe","band"] <- "Tropical Africa"
data_hy[data_hy$band =="Oceania/Asia","band"] <- "Tropical Asia"

data_hy$area10 <- log10(data_hy$area)
data_hy$convex2 <- sqrt(data_hy$convex)

data_hy <- data_hy |> 
  mutate(h = ifelse(source == "Hyperdominants", "Hyperdominants", NA)) |> 
  mutate(nd = ifelse(source == "Non-dominants", "Non-dominants", NA)) |> 
  mutate(d = "Dominance")

data_hy <- data_hy |> 
  dplyr::filter(convex!=0) |> 
  dplyr::filter(region!="Temperate") |> 
  dplyr::filter(growth_form=="tree"|growth_form=="shrub")
...
occplotNB<- data_hy |> 
  ...
  annotate("text",label = expression(R^2 == 0.27 ~  "***" ),
           x=10000,y=7.9,size=4,hjust = 0)+
  annotate("text",label = expression(R^2 == 0.31 ~ "***" ),
...
occplotRS<- data_hy |> 
  ...
  annotate("text",label = expression(R^2 == 0.25 ~  "***" ),
           x=10000,y=2.5,size=4,hjust = 0)+
  annotate("text",label = expression(R^2 == 0.21 ~ "***" ),
...
nf <- dir_ls(here("products","CVs_global_moll_simu"),glob = "*.rds")
a <- read_rds(nf[100])

nf2 <- map_df(nf,readRDS)
colnames(nf2)
nf2 <- nf2[,-52]
nf3 <- nf2 |> drop_na(everything()) 

dd <- dataGlobal |> dplyr::select(species, area, region, band, higher_plant_group)
nf4 <- left_join(dd,nf3, by="species")


nf4p <- nf4 |> pivot_longer(cols = c(matches("convex")),
                            names_to = "cvn",values_to = "cv") |> 
  mutate (type = ifelse(cvn=="convex","observed","simulated"))
...
for(j in 1:length(grep("convex",colnames(nf4)))){
  ...
  #model <- lm(sqrt(td$convex)~log10(td$area))
  s <- summary(model)
